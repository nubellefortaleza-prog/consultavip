...
const SINGLE_LINE_FIELDS: (keyof ReportData)[] = [
  "patientName",
  "consultationDate",
];
...
export default function Home() {
  ...
  const downloadPdfMutation = trpc.consultation.downloadPdf.useMutation();
  const isAdmin = user?.role === "admin";
  const isRecorder = user?.role === "recorder";
  const historyQuery = trpc.consultation.list.useQuery(undefined, {
    enabled: isAuthenticated && showHistory && !isRecorder,
  });
  const settingsQuery = trpc.system.settings.get.useQuery(undefined, {
    enabled: isAuthenticated && isAdmin,
  });
  const updateSettingsMutation = trpc.system.settings.update.useMutation();
  const usersQuery = trpc.system.users.list.useQuery(undefined, {
    enabled: isAuthenticated && isAdmin,
  });
  ...
  const processAudio = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        if (isRecorder) {
          setStep("done");
          toast.success("Transcrição concluída para revisão do admin.");
          return;
        }
    ...
    [uploadMutation, transcribeMutation, generateReportMutation, isRecorder]
  );
  ...
  useEffect(() => {
    if (
      recorder.audioBlob &&
      recorder.audioBlob !== lastBlobRef.current &&
  ...
  }, [consultationId, downloadPdfMutation]);
  ...
  const handleUpdateSettings = useCallback(
    ...
    [updateSettingsMutation, settingsQuery]
  );

  const handleUpdateAiKey = useCallback(async () => {
    if (!aiApiKeyInput.trim()) return;
    try {
      await updateSettingsMutation.mutateAsync({
        aiApiKey: aiApiKeyInput.trim(),
      });
      setAiApiKeyInput("");
      await settingsQuery.refetch();
      toast.success("Chave de IA atualizada");
    } catch (err: any) {
      toast.error(err.message || "Erro ao atualizar chave");
    }
  }, [aiApiKeyInput, updateSettingsMutation, settingsQuery]);

  const handleSetRole = useCallback(
    async (openId: string, role: "user" | "admin" | "recorder") => {
      try {
        await setRoleMutation.mutateAsync({ openId, role });
  ...
  const isProcessing =
    step === "uploading" || step === "transcribing" || step === "sending";
  const stepMessage = useMemo(() => {
    switch (step) {
      case "uploading":
        return "Enviando áudio...";
      case "transcribing":
        return "Transcrevendo consulta com IA...";
      case "sending":
        return "Enviando relatório por e-mail...";
      default:
        return "";
    }
  }, [step]);
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <Card className="max-w-md w-full border-0 shadow-lg bg-white/80 backdrop-blur-sm">
            <CardContent className="p-8 text-center">
              ...
              <h2 className="text-2xl font-semibold mb-3 text-[var(--color-vip-noir)]">
                ConsultaVip
              </h2>
              <p className="text-sm text-[var(--color-vip-noir)]/60 mb-6 font-sans">
                Grave, transcreva e gere relatórios de consultas automaticamente
          ...
          <div className="max-w-2xl mx-auto">
            ...
            {isAdmin && (
              <AdminPanel
                ...
                onWebhookUrlSave={url =>
                  handleUpdateSettings({ webhookUrl: url })
                }
                onToggleGoogleCalendar={checked =>
            ...
            {step === "record" && (
              <RecordingCard
                ...
                hasBlob={!!recorder.audioBlob}
                onProcess={handleProcessStopped}
              />
            )}
            ...
            {isProcessing && (
              <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
                <CardContent className="p-8 md:p-12 text-center">
                  ...
                  <h3 className="text-xl font-semibold text-[var(--color-vip-noir)] mb-2">
                    {stepMessage}
                  </h3>
                  <p className="text-sm text-[var(--color-vip-noir)]/50 font-sans">
                  ...
                  {step === "transcribing" && (
                    <div className="mt-6 flex justify-center gap-1">
                      {[0, 1, 2, 3, 4].map(i => (
                        <div
                          key={i}
                          className="w-1.5 rounded-full bg-[var(--color-vip-blush)] audio-wave-bar"
                          style={{
                ...
                <Card className="border-0 shadow-md bg-white/80 backdrop-blur-sm">
                  <CardContent className="p-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="text-sm font-semibold text-[var(--color-vip-noir)] uppercase tracking-wider font-sans">
                        Transcrição
                      </h3>
                    </div>
                    <div className="bg-[var(--color-vip-pearl)] rounded-lg p-4 max-h-40 overflow-y-auto">
                      <p className="text-sm text-[var(--color-vip-noir)]/70 font-sans leading-relaxed whitespace-pre-wrap">
                ...
                <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
                  <CardContent className="p-6 md:p-8">
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <h3 className="text-lg font-semibold text-[var(--color-vip-noir)]">
                        Relatório da Consulta
                      </h3>
                    </div>
                    <p className="text-sm text-[var(--color-vip-noir)]/50 mb-6 font-sans">
                      Revise e edite os campos abaixo antes de enviar por
                      e-mail.
                    </p>
                        ...
                        <div key={field}>
                          ...
                          {SINGLE_LINE_FIELDS.includes(field) ? (
                            <Input
                              value={report[field]}
                              onChange={e =>
                                updateReportField(field, e.target.value)
                              }
                            ...
                            <Textarea
                              ...
                              rows={3}
                              className="border-[var(--color-vip-silk)] focus:border-[var(--color-vip-blush)] bg-white font-sans resize-y"
                            />
                          )}
                        </div>
                      ))}
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="border-[var(--color-vip-silk)] text-[var(--color-vip-noir)] hover:bg-[var(--color-vip-pearl)] font-sans"
                        size="lg"
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="border-[var(--color-vip-sage)] text-[var(--color-vip-noir)] hover:bg-[var(--color-vip-sage)]/10 font-sans"
                        size="lg"
                      >
            ...
            {step === "done" && (
              <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
                <CardContent className="p-8 md:p-12 text-center">
                  ...
                  <h3 className="text-2xl font-semibold text-[var(--color-vip-noir)] mb-2">
                    {isRecorder ? "Gravação Concluída!" : "Relatório Enviado!"}
                  </h3>
                  <p className="text-sm text-[var(--color-vip-noir)]/50 mb-2 font-sans">
                    {isRecorder ? (
                      "A consulta foi gravada e transcrita. O admin pode finalizar o relatório."
                    ) : (
                      <>
                        O relatório de <strong>{report.patientName}</strong> foi
                  ...
                  {!isRecorder && (
                    <div className="text-left bg-[var(--color-vip-pearl)] rounded-lg p-5 mb-8">
                      <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--color-vip-terracotta)] mb-3 font-sans">
                        Resumo do Relatório
                      </h4>
                      {FIELD_ORDER.map(field => (
                        <div
                          key={field}
                          className="flex gap-2 py-1.5 border-b border-[var(--color-vip-silk)]/50 last:border-0"
                        >
                          <span className="text-xs font-semibold text-[var(--color-vip-noir)]/60 uppercase min-w-[140px] font-sans">
                            {REPORT_LABELS[field]}:
                          </span>
                          <span className="text-xs text-[var(--color-vip-noir)] font-sans">
                            {report[field]}
                          </span>
                        </div>
                      ))}
                  ...
                  {!isRecorder && (
                    <Button
                      onClick={handleDownloadPdf}
                      variant="outline"
                      className="mb-3 border-[var(--color-vip-silk)] text-[var(--color-vip-noir)] hover:bg-[var(--color-vip-pearl)] font-sans"
                      size="lg"
                    >
                      <FileText className="w-4 h-4 mr-2" />
                      Baixar PDF
                  ...
                  <Button
                    onClick={handleNewConsultation}
                    className="bg-[var(--color-vip-blush)] hover:bg-[var(--color-vip-blush)]/90 text-white font-sans"
                    size="lg"
                  >
                    <RotateCcw className="w-4 h-4 mr-2" />
                    Nova Consulta
    ...
    <header className="bg-white/70 backdrop-blur-md border-b border-[var(--color-vip-silk)]/50 sticky top-0 z-50">
      <div className="container flex items-center justify-between h-16 md:h-20">
        <div className="flex items-center gap-3">
          ...
          <span className="hidden sm:block text-xs font-sans font-medium text-[var(--color-vip-noir)]/50 uppercase tracking-widest">
            ConsultaVip
          </span>
        </div>
        {user && (
          <div className="flex items-center gap-2">
            {onToggleHistory && (
              <Button
                ...
                className={`text-xs font-sans ${showHistory ? "text-[var(--color-vip-blush)]" : "text-[var(--color-vip-noir)]/60"}`}
              >
                <History className="w-4 h-4 mr-1" />
                <span className="hidden sm:inline">Histórico</span>
              </Button>
            )}
            <span className="text-xs text-[var(--color-vip-noir)]/50 font-sans hidden md:block">
            ...
            <Button
              ...
              size="sm"
              onClick={onLogout}
              className="text-xs text-[var(--color-vip-noir)]/40 hover:text-[var(--color-vip-noir)] font-sans"
            >
    ...
    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm mb-6">
      <CardContent className="p-6 space-y-5">
        ...
        <div className="space-y-2">
          <Label>Chave da IA (sobrescreve .env)</Label>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              onClick={() => onWebhookUrlSave(webhookInput.trim() || null)}
            >
              Salvar URL
            </Button>
          </div>
          <Button
            variant="outline"
            onClick={() => onToggleWebhook(!settings?.webhookEnabled)}
          >
            Webhook {settings?.webhookEnabled ? "Ativado" : "Desativado"}
          </Button>
        </div>
        <div>
          <Button
            variant="outline"
            onClick={() =>
              onToggleGoogleCalendar(!settings?.googleCalendarEnabled)
            }
          >
            Google Calendar{" "}
            {settings?.googleCalendarEnabled ? "Ativado" : "Desativado"}
          </Button>
        </div>
        <div className="space-y-2">
...
function AppFooter() {
  return (
    <footer className="py-4 text-center border-t border-[var(--color-vip-silk)]/30">
      <p className="text-xs text-[var(--color-vip-noir)]/30 font-sans tracking-wide">
        ConsultaVip &bull; Vip Estetic &copy; {new Date().getFullYear()}
      </p>
    </footer>
  );
}
...
function StepIndicator({ currentStep }: { currentStep: AppStep }) {
  const steps = [
    ...
    { key: "process", label: "Processar" },
    { key: "report", label: "Relatório" },
    { key: "done", label: "Enviado" },
  ];
  const getIdx = () => {
    if (currentStep === "record") return 0;
    if (["uploading", "transcribing"].includes(currentStep)) return 1;
    if (currentStep === "report" || currentStep === "sending") return 2;
    return 3;
  };
    ...
    <div className="flex items-center justify-center gap-2 mb-8">
      {steps.map((s, i) => (
        <div key={s.key} className="flex items-center gap-2">
          <div
            className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-sans font-semibold transition-all ${i <= activeIndex ? "bg-[var(--color-vip-blush)] text-white" : "bg-[var(--color-vip-silk)]/50 text-[var(--color-vip-noir)]/30"}`}
          >
            {i < activeIndex ? <CheckCircle2 className="w-4 h-4" /> : i + 1}
          </div>
          <span
            className={`text-xs font-sans hidden sm:block ${i <= activeIndex ? "text-[var(--color-vip-noir)]" : "text-[var(--color-vip-noir)]/30"}`}
          >
            {s.label}
          </span>
          {i < steps.length - 1 && (
            <div
              className={`w-8 h-px ${i < activeIndex ? "bg-[var(--color-vip-blush)]" : "bg-[var(--color-vip-silk)]"}`}
            />
          )}
        </div>
      ))}
    ...
    <Card className="border-0 shadow-lg bg-white/80 backdrop-blur-sm">
      <CardContent className="p-8 md:p-12 text-center">
        <div className="relative inline-flex items-center justify-center mb-6">
          {isRecording && (
            <div className="absolute inset-0 rounded-full bg-[var(--color-vip-blush)]/20 animate-pulse-recording scale-150" />
          )}
          <button
            onClick={
              isIdle
            ...
            className={`relative w-24 h-24 md:w-28 md:h-28 rounded-full flex items-center justify-center transition-all shadow-lg ${
              isRecording
                ? "bg-[var(--color-vip-blush)] hover:bg-[var(--color-vip-blush)]/90"
                : isPaused
                  ? "bg-[var(--color-vip-terracotta)] hover:bg-[var(--color-vip-terracotta)]/90"
                  : isStopped
                    ? "bg-[var(--color-vip-sage)] hover:bg-[var(--color-vip-sage)]/90"
        ...
        {(isRecording || isPaused || isStopped) && (
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-2xl font-mono font-semibold text-[var(--color-vip-noir)] tracking-wider">
              {formattedDuration}
            </span>
            {isRecording && (
        ...
        {isRecording && (
          <div className="flex justify-center gap-1 mb-6">
            {Array.from({ length: 12 }).map((_, i) => (
              <div
                key={i}
                className="w-1 rounded-full bg-[var(--color-vip-blush)] audio-wave-bar"
        ...
        <div className="flex justify-center gap-3">
          {isRecording && (
            <Button
              onClick={onPause}
              variant="outline"
              className="border-[var(--color-vip-silk)] text-[var(--color-vip-noir)] font-sans"
            >
          ...
          {isPaused && (
            <>
              <Button
                onClick={onResume}
                className="bg-[var(--color-vip-terracotta)] hover:bg-[var(--color-vip-terracotta)]/90 text-white font-sans"
              >
              ...
              <Button
                onClick={onStop}
                variant="outline"
                className="border-[var(--color-vip-blush)] text-[var(--color-vip-blush)] font-sans"
              >
    ...
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Button
          ...
          size="sm"
          onClick={onBack}
          className="text-[var(--color-vip-noir)]/60 font-sans"
        >
          ← Voltar
        </Button>
        <h2 className="text-xl font-semibold text-[var(--color-vip-noir)]">
          Histórico de Consultas
        </h2>
      </div>
      {loading ? (
        <div className="text-center py-12">
          <Loader2 className="w-6 h-6 animate-spin text-[var(--color-vip-blush)] mx-auto" />
        </div>
        ...
        <div className="space-y-3">
          {consultations.map((c: any) => (
            <Card
              key={c.id}
              className="border-0 shadow-md bg-white/80 hover:shadow-lg transition-shadow"
            >
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="font-semibold text-[var(--color-vip-noir)] text-sm">
                      {c.patientName || "Paciente não identificado"}
                    </h4>
                    <p className="text-xs text-[var(--color-vip-noir)]/50 font-sans mt-1">
                      {c.consultationDate ||
                        new Date(c.createdAt).toLocaleDateString("pt-BR")}
                    </p>
                  </div>
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-sans ${c.emailSent === "yes" ? "bg-[var(--color-vip-sage)]/20 text-[var(--color-vip-sage)]" : "bg-[var(--color-vip-silk)]/50 text-[var(--color-vip-terracotta)]"}`}
                  >
                    {c.emailSent === "yes" ? "Enviado" : "Pendente"}
                  </span>
                </div>
                {c.mainComplaints && c.mainComplaints !== "Não mencionado" && (
                  <p className="text-xs text-[var(--color-vip-noir)]/40 font-sans mt-2 line-clamp-2">
                    {c.mainComplaints}
                  </p>
                )}
              </CardContent>
